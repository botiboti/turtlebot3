...
bool controlMotors(const float wheel_radius, const float wheel_separation, float* value)
{
  ...
  dxl_comm_result = motor_driver.writeVelocity((int64_t)wheel_velocity_cmd[LEFT], (int64_t)wheel_velocity_cmd[RIGHT]);
...
void commandVelocitiesCallback(const geometry_msgs::Twist& cmd_vels_msg)
{
  ...
  goal_velocities_from_cmd[1] = cmd_vels_msg.linear.y;
